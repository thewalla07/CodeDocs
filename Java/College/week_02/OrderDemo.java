...
class OrderDemo {
   ...
   public static void main(String[] args)
   {
      ...
      for (Printable p: points) print(p);
      Time[] times = {new Time(15,10), new 
				Time(11,5), new Time(9,20)};
      sort(times); // same sort() as for points!
      for (Printable p: times) print(p);
	  
	  Printable[] mixed = { new Point(3,5),new Time(11,5),new Point(5,4), new Time(9,20)};
	  for(Printable p: mixed) print(p);
  }
}
